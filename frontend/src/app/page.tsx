...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white/60 backdrop-blur-[1px] text-black relative bg-[url('/grid.svg')] bg-[length:40px_40px] sm:bg-[length:50px_50px] bg-center">
      ...
      {/* LOGO (signout only if user signed in) */}
      <button
      ...
      >
        <img src="/logo.png" className="h-6 w-6 sm:h-7 sm:w-7" alt="Vibeboard logo" />
        <span className="font-[Instrument_Serif] text-black text-xl sm:text-2xl">
          Vibeboard
        </span>
      </button>

      {/* RIGHT BUTTON depends on auth */}
      {isSignedIn ? (
        <button
      ...
      {/* CENTER TITLE */}
      <div className="flex flex-col items-center justify-center pt-28 sm:pt-32 px-4 text-center">
        <h1 className="font-[Instrument_Serif] text-black font-normal text-[48px] sm:text-[78px]">
          Vibeboard
        </h1>
        <p className="mt-4 sm:mt-6 text-[16px] sm:text-[20px] text-black/70 max-w-[90%]">
          Find your visual direction instantly. Spend less time scrolling, more time designing.
        </p>
      </div>

      {/* SEARCH BOX */}
      <div className="mx-auto w-[92%] max-w-4xl rounded-[22px] bg-white p-7 mt-10 border-[7px] border-[#F7F7F7] shadow">
        <input
          type="text"
          placeholder="Describe your vibe (e.g., calm fintech dashboard, bold landing page)"
          value={inputValue}
          onChange={(e) => setInputValue(e.target.value)}
          className="w-full bg-transparent outline-none text-[17px] placeholder:text-gray-500"
        />
        ...
        <div className="mt-6 flex items-center flex-wrap gap-3">
          ...
          <button
            ...
            className="w-[136px] h-[48px] text-white font-semibold bg-gradient-to-b from-black to-[#484848] border-[4px] border-[#1C1C1C] rounded-[10px] shadow ml-auto hover:scale-[1.05] transition"
          >
            Vibe Search
          </button>
        </div>
