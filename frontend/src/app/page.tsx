...
export default function Home() {
 ...
 const handleTagClick = (tag: string) => {
  // if input empty → set directly
  if (inputValue.trim() === "") {
    setInputValue(tag);
  } 
  // if input already has words → append with space
  else {
  ...
  return (
    <div className="min-h-screen bg-white/60 backdrop-blur-[1px] text-black relative bg-[url('/grid.svg')] bg-[length:50px_50px] bg-center">
      ...
      <button
        ...
        className="absolute top-6 right-10 px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-xl text-sm font-medium shadow-sm"
      >
        Start for free
      </button>
      ...
      <div className="flex flex-col items-center justify-center pt-32">
        <h1 className="font-[Instrument_Serif] text-black font-normal text-[78px] leading-none tracking-tight">
          Vibeboard
        </h1>
      ...
      {/* big search box */}
      <div
        className="mx-auto w-full max-w-4xl rounded-[22px] bg-white p-7 mt-10 border border-[#e5e5e5]
